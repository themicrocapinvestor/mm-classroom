---
layout: default
title: "What a screener does: a filter for ten thousand stocks"
module: 9
chapter: 1
permalink: /m09/c01/
---

<div class="crumb"><a href="/mm-classroom/">Classroom</a> / <a href="/mm-classroom/m09/">Module 9: Screener Craft</a> / Chapter 1</div>

# What a screener does: a filter for ten thousand stocks

<div class="learn" markdown="1">

**What you will learn**

- What a stock screener is, in one plain sentence
- Why the US market is too big to read company by company
- How a few conditions turn ten thousand names into a shortlist
- Why a shortlist is a list of questions, not answers

</div>

Open any shopping app and type "cricket bat". You get nine thousand results. So you tick the filters on the side: price under ₹3,000, rated four stars and above, English willow. Twenty bats remain. You have not bought anything. You have only made the pile small enough to look at.

A stock screener does exactly this for companies. A stock screener is a tool that filters a long list of companies by conditions you set, such as size, price, or profit, and shows you the names that pass. That is all it does, and that is enough to make it one of the most useful free tools on the internet.

## Why you need one

The US market lists thousands of companies. Count the very small ones traded "over the counter" (meaning away from the big exchanges, often tiny firms with little public information) and the total is around ten thousand. Nobody can read ten thousand annual reports. Even one report a day would take about 27 years, and by then the first company would have changed beyond recognition.

So every serious reader of companies starts the same way: with a sieve. First you shrink the market to a shortlist, then you spend your reading hours only on the shortlist.

## See it working

The screener on [StockAnalysis](https://stockanalysis.com/stocks/screener/?ref=MICROCAPMINUTE) is free and needs no login. It gives you boxes where you type conditions. Ask for companies worth more than $10 billion (about ₹90,000 crore) and thousands of names shrink to a few hundred. Add "must be profitable" and the pile shrinks again.

![The StockAnalysis screener with its filter boxes]({{ "/assets/shots/sa-screener.png" | relative_url }})

Notice the filter boxes: every condition you type removes companies, and the result list updates immediately.

Click any name in the results and you land on that company's own page, with its price, its size, and its key numbers, one company at a time. Here is Apple's:

![Apple's overview page on StockAnalysis]({{ "/assets/shots/sa-aapl-overview.png" | relative_url }})

Notice how the screen has brought you to one company's door; the reading starts from here.

## The one warning that matters most

A screener has read nothing. It matches numbers that other people typed in, and it does not know why any number is high or low. Passing a screen is like clearing the cut-off in an entrance exam: it gets you to the interview, not the seat. That is why, in this classroom, every name that passes a screen still gets checked against the company's own filings before it earns your time. Chapter 5 of this module shows you how.

<div class="try" markdown="1">

**Try it yourself**

Open the [StockAnalysis screener](https://stockanalysis.com/stocks/screener/?ref=MICROCAPMINUTE). Set one condition: market value above $100 billion (market value, also called market capitalisation, is the total price of all of a company's shares put together). Count the results. Now add a second condition: positive profit this year. Count again. Write down both numbers and one sentence about what the second filter removed.

</div>

<div class="takeaways" markdown="1">

**Key takeaways**

- A screener filters a huge list of stocks by conditions you set, like the filters on a shopping app.
- The US market has thousands of listed companies; a screener makes the pile readable.
- A shortlist is a list of candidates to check, never a list of things to buy.
- The screener itself has read nothing; the real checking happens later, in the filings.

</div>

**Read one real thing**

[Apple's page on SEC EDGAR](https://www.sec.gov/edgar/browse/?CIK=320193&owner=exclude), the US regulator's public filing cabinet: notice how many documents one company files in a single year. The screener has opened none of them.

<div class="pager">
<span>Previous: <a href="/mm-classroom/m09/">Module 9 index</a></span>
<span>Next: <a href="/mm-classroom/m09/c02/">Chapter 2: Value, quality, growth filters</a></span>
</div>
